...
try:
    ...
    raise ImportError(
        ...
        "/Library/Frameworks/Python.framework/Versions/3.14/bin/python3 -m pip install reportlab"
    ) from exc
...
def _build_pdf(
    ...
    route_text = _route_title(decoration_type, decoration_color)
    ...
    info = Table(
        ...
        colWidths=[2.4 * inch, 3.4 * inch, 2.2 * inch, 2.1 * inch],
    )
    ...
    for (style, product, garment_color), item_group in route_group.groupby(
        ...
        breakdown = Table(breakdown_rows, colWidths=[3.4 * inch, 0.8 * inch])
